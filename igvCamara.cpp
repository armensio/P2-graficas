...
void igvCamara::orbitar(float valor1, float valor2) {
    ...
    // Convertimos a coordenadas esféricas
    float angulo1 = atan2(dir[Z], dir[X]);  // ángulo horizontal
    float angulo2 = asin(dir[Y] / radio);     // ángulo vertical

    // Aplicamos desplazamientos
    angulo1 += valor2;
    angulo2 += valor1;

    // Limitamos phi para evitar invertir la cámara
